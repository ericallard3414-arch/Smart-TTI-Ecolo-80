...
class Ecolo80ModelSelect : public select::Select {
 public:
...
class Ecolo80Component : public Component {
 public:
  ...
  void set_current_consumption_sensor(sensor::Sensor *value) {
    this->current_consumption_sensor_ = value;
  }
  void set_power_consumption_sensor(sensor::Sensor *value) {
    this->power_consumption_sensor_ = value;
  }
  void set_model_select(Ecolo80ModelSelect *value) { this->model_select_ = value; }
  void set_model_index(size_t index, bool save);
 ...
 protected:
  ...
  void publish_consumption_state_(bool heating);
  void apply_model_index_(size_t index);
  ...
  sensor::Sensor *power_consumption_sensor_{nullptr};
  Ecolo80ModelSelect *model_select_{nullptr};

  std::string model_name_{"ECOLO 80"};
  float rated_current_amps_{18.0f};
  size_t model_index_{2};
  ESPPreferenceObject model_preference_;
  uint32_t last_consumption_publish_ms_{0};
  static constexpr float SUPPLY_VOLTAGE = 240.0f;
  static constexpr uint32_t CONSUMPTION_REFRESH_MS = 5000;
